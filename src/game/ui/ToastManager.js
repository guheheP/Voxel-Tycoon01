...
export class ToastManager {
  constructor() {
    ...
    // イベント購読
    eventBus.on('item:sold',        (d) => this.show(`${d.item.name} が ${d.item.value}G で売れました！`, 'gold'));
    eventBus.on('adventurer:return', (d) => this.show(`${d.adventurer.name} が帰還！ ${d.loot.length}個の素材を入手`, 'green'));
    eventBus.on('day:newDay',       (d) => this.show(`${d.day}日目 — 維持費 ${d.rent}G を支払いました`, 'default'));
    eventBus.on('rank:up',          (d) => this.show(`🎉 ランクアップ！ ${d.rankName}`, 'special'));
    eventBus.on('recipe:unlocked',  (d) => this.show(`📜 新レシピ解放: ${d.name}`, 'green'));
    eventBus.on('area:unlocked',    (d) => this.show(`🗺️ 新エリア解放: ${d.name}`, 'green'));
    eventBus.on('customer:arrived', (d) => this.show(`${d.icon} ${d.name}が来店！「${d.dialogue}」`, 'default'));
    eventBus.on('customer:left',    (d) => this.show(`${d.name}は帰ってしまった…`, 'red'));
    eventBus.on('customer:bought',  (d) => this.show(`${d.customerName}が ${d.item.name} を ${d.price}G で購入！`, 'gold'));
    eventBus.on('adventurer:levelUp', (d) => this.show(`⬆️ ${d.adventurer.name} が Lv.${d.newLevel} になった！`, 'special'));
